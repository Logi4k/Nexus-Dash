    ...
    <Modal open={open} onClose={onClose} title={title} size="md">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <CustomSelect
            value={form.type}
            onChange={(v) => setField("type", v as "etf" | "stock")}
            options={[
              { value: "etf", label: "ETF" },
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <CustomSelect
              ...
              onChange={(v) => setField("frequency", v as Subscription["frequency"])}
              options={[
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <DatePicker
            ...
            onChange={(v) => setField("nextRenewal", v)}
          />
        </div>
        <div className="flex flex-col gap-1.5">
        ...
        <div className="progress-track mt-1">
          <div
            ...
            style={{
              ...
              background: stats.totalPnl >= 0 ? "var(--color-profit)" : "var(--color-loss)",
            }}
          />
        </div>
      </div>
...
export default function InvestmentsPage() {
  ...
  const offlineMode =
    ...
    window.localStorage.getItem(OFFLINE_MODE_KEY) === "true";

  // Device-only API key stays out of synced app data and cloud payloads.
  const [deviceApiKey, setDeviceApiKey] = useState(() => getT212ApiKey());
  const apiKey = deviceApiKey || (import.meta.env.VITE_T212_API_KEY as string | undefined);

  // ── T212 sync state ───────────────────────────────────────────────────────
  const [syncing, setSyncing] = useState(false);
  const [syncError, setSyncError] = useState<string | null>(null);
  const [connected, setConnected] = useState<boolean>(!!apiKey && !offlineMode);
  ...
  const emptyTargetForm = { emoji: "TG", name: "", desc: "", target: "0", saved: "0", monthly: "" };
  ...
  function saveTarget() {
    ...
    toast.success(editingTargetId ? "Goal updated" : "Goal added");
  ...
  const syncT212 = useCallback(async () => {
    ...
    try {
      ...
      // Validate response types before parsing
      const posContentType = posRes.headers.get("content-type") ?? "";
      const cashContentType = cashRes.headers.get("content-type") ?? "";
      if (!posContentType.includes("json") || !cashContentType.includes("json")) {
        throw new Error("T212 returned an unexpected response format. The API may be down.");
      }

      const positionsRaw: unknown = await posRes.json();
      const cashRaw: unknown = await cashRes.json();

      if (!Array.isArray(positionsRaw)) {
        throw new Error("T212 portfolio response was not an array — API format may have changed.");
      }

      // Validate each position has required fields
      const positions: T212Position[] = positionsRaw.map((p) => {
        if (
          typeof p !== "object" ||
          p === null ||
          typeof (p as T212Position).ticker !== "string" ||
          typeof (p as T212Position).quantity !== "number" ||
          typeof (p as T212Position).averagePrice !== "number" ||
          typeof (p as T212Position).currentPrice !== "number"
        ) {
          throw new Error(
            `T212 position data is incomplete or malformed (ticker: "${(p as T212Position).ticker}"). API format may have changed.`
          );
        }
        return p as T212Position;
      });

      if (typeof cashRaw !== "object" || cashRaw === null) {
        throw new Error("T212 cash response was not an object — API format may have changed.");
      }

      const cash: T212Cash = {
        free: (cashRaw as T212Cash).free ?? 0,
        total: (cashRaw as T212Cash).total ?? 0,
        ppl: (cashRaw as T212Cash).ppl ?? 0,
        result: (cashRaw as T212Cash).result ?? 0,
        invested: (cashRaw as T212Cash).invested ?? 0,
      };
  ...
  useEffect(() => {
    if (!offlineMode && apiKey && t212.last_sync === 0) {
      syncT212();
    }
  }, [apiKey, offlineMode, syncT212, t212.last_sync]);
        ...
        <div className="flex flex-col gap-6 xl:gap-7">
          ...
          <div className="card p-5 flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3 md:hidden">
              ...
              {filteredInvestments.length === 0 && investments.length > 0 && (
                <div className="rounded-2xl border border-[rgba(var(--border-rgb),0.06)] bg-[rgba(var(--border-rgb),0.02)] px-4 py-6 text-center text-sm text-tx-3">
                  No holdings found
                </div>
              )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
                      <div className="rounded-xl border border-[rgba(var(--border-rgb),0.05)] bg-[rgba(var(--border-rgb),0.03)] p-2.5">
                    ...
                    <div className="mt-3 rounded-xl border border-[rgba(var(--border-rgb),0.05)] bg-[rgba(var(--border-rgb),0.03)] p-3">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-2 font-mono font-bold"
                          style={{ borderLeft: `2.5px solid ${accentColor}50`, color: accentColor === "var(--color-profit)" ? "var(--color-profit)" : "var(--color-loss)" }}>
            ...
            {wealthTargets.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                {wealthTargets.map((wt) => {
                  ...
                  const ringColor = progressPct >= 100 ? "var(--color-teal)" : progressPct >= 66 ? "var(--color-warn)" : progressPct >= 33 ? "var(--color-teal)" : "var(--color-indigo)";
                    ...
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-0.5 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                          <button
                            className="p-1.5 rounded-lg text-tx-4 hover:text-tx-1 hover:bg-[rgba(var(--border-rgb),0.06)] transition-colors"
              ...
              <>
                ...
                <div className="flex flex-col gap-2 mb-3">
                  {activeSubs.slice(0, showAllSubs ? undefined : 5).map((sub) => {
                    ...
                    const isUrgent = isFinite && days >= 0 && days <= 7;
                    const isUpcoming = isFinite && days >= 0 && days <= 14 && !isUrgent;
                    const shareOfTotal = totalMonthlySubs > 0 ? (monthlyCost / totalMonthlySubs) * 100 : 0;
                    const urgentColor = isUrgent ? "var(--color-loss)" : isUpcoming ? "var(--color-warn)" : undefined;
                    return (
                      <div
                        key={sub.id}
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex gap-2">
              <input
                className="nx-input w-14 text-center text-2xl px-2 py-1 leading-none"
                value={targetForm.emoji}
                readOnly
                placeholder="—"
                title="Select an emoji below"
              />
              <input
