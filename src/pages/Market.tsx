        ...
        {MARKET_SESSIONS.map((session) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style={{ color: isActive ? session.color : isNext ? `${session.color}bb` : "#52525b" }}
                  >
                    {session.name}
                  </span>
                </div>
              ...
              {/* Time range */}
              <div className="text-tx-3 text-xs font-mono mb-1">
                {session.startET} &ndash; {session.endET}{" "}
                <span className="text-tx-4">ET</span>
              </div>
          ...
          <div className="flex rounded-lg overflow-hidden border border-white/[0.08]">
            {(["all", "High", "Medium"] as const).map((v) => (
              <button
                ...
                className={cn(
                  "px-2.5 py-1 text-[10px] font-medium transition-colors",
                  filterImpact === v ? "bg-white/10 text-tx-1" : "text-tx-4 hover:text-tx-3",
                )}
              >
          ...
          <div className="flex rounded-lg overflow-hidden border border-white/[0.08]">
            {(["all", "USD", "GBP"] as const).map((v) => (
              <button
                ...
                className={cn(
                  "px-2.5 py-1 text-[10px] font-medium transition-colors",
                  filterCountry === v ? "bg-white/10 text-tx-1" : "text-tx-4 hover:text-tx-3",
                )}
              >
                {v === "all" ? "All" : v}
              </button>
            ...
            {grouped.map(([dateKey, dayEvs]) => {
              ...
              return (
                <div key={dateKey}>
                  ...
                  {/* Events list */}
                  <div className="rounded-xl overflow-hidden border border-white/[0.06] divide-y divide-white/[0.04]">
                    {dayEvs.map((ev, i) => {
...
function EconomicCalendar() {
  ...
  const events: EconomicEvent[] = (data as any).economicEvents ?? [];
  ...
  function handleAdd() {
    ...
    update((prev) => ({
      ...prev,
      economicEvents: [...((prev as any).economicEvents ?? []), ev],
    }));
    setAddOpen(false);
    setForm({
      date: new Date().toISOString().slice(0, 10),
  ...
  function handleDelete(id: string) {
    update((prev) => ({
      ...prev,
      economicEvents: ((prev as any).economicEvents ?? []).filter(
          ...
          {events.length === 0 ? (
            ...
            <div className="space-y-2">
              {grouped.map(({ date, items }) => {
                ...
                return (
                  <div key={date} className="card overflow-hidden border-white/[0.06]">
                    <button
                      ...
                      <div className="border-t border-white/[0.05] divide-y divide-white/[0.04]">
                        {items.map((ev) => {
                          ...
                          return (
                            <div
                              key={ev.id}
                              className="flex items-center gap-3 px-4 py-3 group hover:bg-white/[0.02] transition-colors"
                            >
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Economic Event" size="sm">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-tx-3 text-xs block mb-1.5">Time (ET)</label>
              <input
                className="nx-input"
                type="time"
                value={form.time}
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((p) => ({
                  ...p,
                  impact: e.target.value as EconomicEvent["impact"],
                }))
              }
            >
...
function PositionSizer() {
  ...
  return (
    <div className="card p-4">
      ...
      {/* Contract info */}
      <div className="mt-2 flex items-center gap-3 text-[10px] text-tx-4 pt-2 border-t border-white/[0.05]">
