      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <DatePicker
            ...
            onChange={(date) => setField("nextPayment", date)}
          />
        </div>
...
function DebtRow({
  ...
  return (
    <div className={cn("card overflow-hidden", bw && "card--parchment-panel")}>
      ...
      {/* Mobile stats row */}
      <div className="sm:hidden flex flex-wrap items-center gap-x-4 gap-y-1 px-4 pb-3 border-t border-[rgba(var(--border-rgb),0.05)] pt-2">
      ...
      {/* Expanded details */}
      {expanded && (
        <div className="border-t border-[rgba(var(--border-rgb),0.05)] p-4">
          {/* Utilization + limits */}
          ...
          {(debt.payments ?? []).length === 0 ? (
            ...
            <div className="flex flex-col gap-1.5 max-h-40 overflow-y-auto">
              {(debt.payments ?? []).map((p) => (
                <div key={p.id} className="flex items-center justify-between text-xs px-3 py-2 rounded-lg bg-[rgba(var(--border-rgb),0.03)] border border-[rgba(var(--border-rgb),0.05)]">
...
function StrategyPanel({ debts }: { debts: Debt[] }) {
  ...
  return (
    <div className={cn("card p-5 flex flex-col gap-4", isBW && "card--parchment-panel")}>
      ...
      {/* Extra monthly input */}
      <div className="flex flex-col gap-1.5 p-3 rounded-xl bg-[rgba(var(--border-rgb),0.04)] border border-[rgba(var(--border-rgb),0.07)]">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {/* Avalanche */}
        <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: "rgba(var(--color-warn),0.04)", border: "1px solid rgba(var(--color-warn),0.15)" }}>
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg" style={{ background: "rgba(var(--color-warn),0.10)" }}>
              <Zap size={12} style={{ color: "var(--color-warn)" }} />
            </div>
            <div>
              <p className="text-xs font-semibold" style={{ color: "var(--color-warn)" }}>Avalanche</p>
          ...
          <div className="text-[10px] text-tx-4 pt-1 border-t border-[rgba(var(--border-rgb),0.05)]">
            ✓ Best for minimising total interest paid
          </div>
        </div>

        {/* Snowball */}
        <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: "rgba(var(--color-blue),0.04)", border: "1px solid rgba(var(--color-blue),0.15)" }}>
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg" style={{ background: "rgba(var(--color-blue),0.10)" }}>
              <Snowflake size={12} style={{ color: "var(--color-blue)" }} />
            </div>
            <div>
              <p className="text-xs font-semibold" style={{ color: "var(--color-blue)" }}>Snowball</p>
      ...
      <div>
        ...
        {[...debts]
          ...
          .map((d, i) => {
            ...
            return (
              <div key={d.id} className="flex items-center gap-2 py-1.5 border-b border-[rgba(var(--border-rgb),0.04)] last:border-0">
        ...
        <div className="flex flex-col gap-5 xl:gap-6">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard
              ...
              icon={<BadgePoundSterling size={15} />}
              accentColor="var(--color-teal)"
              delay={60}
            />
            <StatCard
              label="Utilisation"
              value={stats.overallUtil}
              prefix=""
              suffix="%"
              decimals={1}
              subLabel={stats.overallUtil > 30 ? "Above 30%" : "Healthy"}
              icon={<Zap size={15} />}
              accentColor={stats.overallUtil >= 70 ? "var(--color-loss)" : stats.overallUtil >= 30 ? "var(--color-warn)" : "var(--color-profit)"}
              delay={120}
            />
            <StatCard
              label="Mo. Interest"
              value={stats.totalInterest}
              subLabel="Cost of carrying"
              icon={<Clock size={15} />}
            ...
            <div className={cn("card p-5", isBW && "card--parchment-panel")}>
              ...
              <div className="flex flex-col gap-3 md:hidden">
                {debts.map((d) => {
                  ...
                  return (
                    <div key={d.id} className="rounded-2xl border border-[rgba(var(--border-rgb),0.06)] bg-[rgba(var(--border-rgb),0.02)] p-4">
                ...
                <div className="rounded-2xl border border-[rgba(var(--border-rgb),0.06)] bg-[rgba(var(--border-rgb),0.02)] p-4">
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody className="divide-y divide-[rgba(var(--border-rgb),0.04)]">
                    {debts.map((d) => {
                      ...
                      const acc = bwAccent(d.name);
                      return (
                        <tr key={d.id} className="group hover:bg-[rgba(var(--border-rgb),0.02)] transition-colors">
                    ...
                    {/* Total row */}
                    <tr className="border-t border-[rgba(var(--border-rgb),0.08)]">
              ...
              {(() => {
                ...
                return (
                  <div className="mt-5 pt-4 border-t border-[rgba(var(--border-rgb),0.06)]">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="relative shrink-0">
                    <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ transform: "rotate(-90deg)" }}>
                      ...
                      <circle cx={CX} cy={CY} r={R} fill="none" stroke="rgba(var(--border-rgb),0.05)" strokeWidth="7" />
            ...
            return (
              <div className={cn("card p-4", isBW && "card--parchment-panel")}>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative shrink-0">
                    <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ transform: "rotate(-90deg)" }}>
                      <circle cx={CX} cy={CY} r={R} fill="none" stroke="rgba(var(--border-rgb),0.05)" strokeWidth="11" />
                      {segments.map((seg) => (
                        <circle
                          key={seg.name}
