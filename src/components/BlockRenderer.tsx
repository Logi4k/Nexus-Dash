...
function TextArea({
  ...
  return (
    <textarea
      ...
      style={{ ...baseInputStyle, ...style, minHeight }}
      onChange={(e) => onChange(e.target.value)}
      onKeyDown={onKeyDown as React.KeyboardEventHandler<HTMLTextAreaElement>}
      onFocus={onFocus}
      onInput={(e) => {
        const el = e.currentTarget;
        const computedMinHeight = parseFloat(getComputedStyle(el).minHeight) || 0;
        el.style.height = "auto";
        const newHeight = Math.max(computedMinHeight, el.scrollHeight);
    ...
    case "todo":
      return (
        <div className="flex gap-3 items-start">
          ...
          >
            {block.checked && <span className="text-[9px] font-black" style={{ color: "var(--bg-base)" }}>OK</span>}
          </button>
          <TextArea
            value={block.content}
            autoFocus={autoFocus}
    ...
    default:
      return (
        <TextArea
          ...
          style={bodyStyle}
          minHeight="2rem"
          placeholder="Type '/' for commands or Ctrl+Enter for a new section..."
        />
      );
  }
}
