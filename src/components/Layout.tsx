...
import PropRulesDrawer from "./PropRulesDrawer";
...
export default function Layout() {
  ...
  const [propRulesOpen, setPropRulesOpen] = useState(false);
  ...
  const commandItems = useMemo<CommandPaletteItem[]>(
    () => [
      ...
      {
        id: "prop-rules",
        label: "Prop Trading Rules",
        description: "View all prop firm rules at a glance",
        group: "Reference",
        keywords: ["firms", "drawdown", "limits", "payout"],
        Icon: Scale,
        run: () => setPropRulesOpen(true),
      },
    ],
    [navigate, setPropRulesOpen]
  );
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-bg-base">
      <div className="hidden md:flex h-full flex-shrink-0">
        <Sidebar onOpenCommandPalette={() => setCommandPaletteOpen(true)} onOpenPropRules={() => setPropRulesOpen(true)} />
      </div>
      ...
      <PropRulesDrawer
        open={propRulesOpen}
        onClose={() => setPropRulesOpen(false)}
      />
    </div>
  );
}
