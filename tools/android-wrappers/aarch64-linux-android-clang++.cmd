...
for /f "delims=" %%i in ('dir /b /ad /o-n "%NDK_ROOT%" 2^>nul') do (
  ...
  if exist "!NDK_BIN!\aarch64-linux-android24-clang++.cmd" goto run
)
echo [android-wrappers] ERROR: aarch64-linux-android24-clang++.cmd not found under %NDK_ROOT% >&2
exit /b 1
:run
call "!NDK_BIN!\aarch64-linux-android24-clang++.cmd" %*
